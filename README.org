...
*** Flake Registries
    ...
    [[https://nixos.org/manual/nix/stable/command-ref/new-cli/nix3-registry.html][Flake registries]] contain flakes, which are used when we specify packages
    using shorthand paths such as nixpkgs#hello.
